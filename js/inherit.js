(function () {
  const root = document.getElementById("inherit-app");
  if (!root) return;
  ...
  async function load() {
    ...
    const res = await fetch("data/inherit-lookup.json");
  ...
  function paintRoom() {
    ...
    root.querySelector("[data-heir-name]").textContent = heir ? heir.name : "Next man";
  ...
  load().then(paint).catch(function () {
    root.querySelector("[data-verdict]").textContent = "The next man could not load. Refresh the page.";
  });
})();
